...
public class VoitureController {
    ...
    public ResponseEntity<Object> findAll() {
        try {
            ...
            voitures.forEach(voiture -> {
                if (voiture.getId_client() != null) {  // Utilise getId_client()
                    try {
                        voiture.setClient(clientService.clientById(voiture.getId_client()));
                    } catch (Exception e) {
                        System.err.println("Client not found for ID: " + voiture.getId_client());
                    }
                }
            });
    ...
    public ResponseEntity<Object> findById(@PathVariable Long id) {
        try {
            ...
            // Récupère le client via Feign et l'associe
            voiture.setClient(clientService.clientById(voiture.getId_client()));
    ...
    public ResponseEntity<List<Voiture>> findByClient(@PathVariable Long id) {
        try {
            ...
            if (client != null) {
                ...
                // Ajoute cette partie pour remplir le client dans chaque voiture
